...
@TeleOp(name="CameraTesting", group = "auto")
//@Disabled
public class CameraTesting extends LinearOpMode {
    ...
    public VectorF getRobotPosition(VectorF posPC, VectorF posWC){
        float newX = 0, newY = 0, newZ = 0;
        int i = 0;
        if(posPC != null || posWC != null){
            if(posPC != null){
                i++;
                newX += posPC.get(0);
                newY += posPC.get(1);
            ...
            telemetry.addData("y: ", newX);
            telemetry.addData("z: ", newX);
            telemetry.update();

            return new VectorF(newX,newY,newZ);
        }
        return null;
    }
    ...
    public VectorF roboPosFromPC(){
        ...
        for (VuforiaTrackable trackable : trackablesPC) {
            if (((VuforiaTrackableDefaultListener) trackable.getListener()).isVisible()) {
                ...
                if (robotLocationTransform != null) {
                    lastLocPC = robotLocationTransform;
                }
                break;
            }
        }
        ...
        if (targetVisible) {
            telemetry.update();
            return lastLocPC.getTranslation();
        }
        else {
